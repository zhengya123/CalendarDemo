...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        self.calendar = calendar;
            ...
            nextBtn.frame = CGRectMake(20, 15, 20, 20);
            [nextBtn addTarget:self action:@selector(nextClick:) forControlEvents:UIControlEventTouchUpInside];
            [self.calendar addSubview:nextBtn];
        
            UIButton * nextBtn1 = [UIButton buttonWithType:UIButtonTypeCustom];
            [nextBtn1 setTitle:@"》" forState:UIControlStateNormal];
            [nextBtn1 setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            nextBtn1.frame = CGRectMake(self.frame.size.width - 40, 15, 20, 20);
            [nextBtn1 addTarget:self action:@selector(next1Click:) forControlEvents:UIControlEventTouchUpInside];
            [self.calendar addSubview:nextBtn1];
